...
import { compressImage, fileToBase64 } from './image-utils'
...
export function syncProductAdd(product) {
  const normalized = {
    ...product,
    talles: Array.isArray(product.talles)
      ? product.talles.join(', ')
      : (product.talles || ''),
    imagenURL: Array.isArray(product.imagenes)
      ? product.imagenes.join('|')
      : (product.imagenURL || ''),
    disponible: product.disponible === false ? 'FALSE' : 'TRUE',
  }
  const mapped = mapKeys(normalized, PRODUCT_FIELD_MAP)
  return callSync({ action: 'add_product', item: mapped })
}

export async function syncImageUpload(file, options) {
