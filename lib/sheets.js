...
import { productos, testimonios, lookbookItems, faqs, temporadaActual, promos } from './defaults'

const SHEETS_BASE = `https://docs.google.com/spreadsheets/d/${SHEET_ID}/gviz/tq?tqx=out:json&headers=1&sheet=`
...
export async function getProductos() {
  ...
  return rows
    .filter((row) => row.Nombre && row.Disponible !== false && row.Disponible !== 'FALSE')
    .map((row) => ({
      id: row.ID,
      nombre: row.Nombre,
      ...
      talles: row.Talles != null
        ? String(row.Talles).split(',').map((t) => t.trim()).filter(Boolean)
        : [],
      badge: row.Badge || null,
      imagenes: row.ImagenURL ? String(row.ImagenURL).split('|').map((u) => getDriveImageUrl(u.trim())).filter(Boolean) : [],
...
export async function getTestimonios() {
  ...
  return rows
    .filter((row) => row.Nombre && row.Texto)
    .map((row) => ({
      id: row.ID,
      nombre: row.Nombre,
      texto: row.Texto,
...
export async function getLookbook() {
  ...
  return rows
    ...
    .map((row) => ({
      ...
      imagenUrl: row.ImagenURL ? getDriveImageUrl(row.ImagenURL.trim()) : null,
      placeholder: `linear-gradient(135deg, #2B3A52, #6B7B8D)`,
    }))
}
...
export async function getFAQs() {
  ...
  return rows
    .filter((row) => row.Pregunta && row.Respuesta)
    .map((row) => ({
